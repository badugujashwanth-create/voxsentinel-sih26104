...
Render plans that sleep create `SIH_DEMO_COLD_START_RISK`. Warm both Render services 15 minutes before judging, run one health request and one controlled inference, open Vercel, verify WSS, then run a short LIVE acceptance before the presentation.
