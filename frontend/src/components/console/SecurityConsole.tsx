...
function EvidenceRail({ event, level, state }: { event: DemoController["currentEvent"]; level: string; state: string }) { const calm = state === "CALM"; const isUnavailable = (field: string) => event?.evidence_availability?.[field] === "NOT_EVALUATED"; const uncalibratedScore = event?.synthetic_score_semantics === "uncalibrated"; const evidence = event ? [{ field: "synthetic_probability", label: "Synthetic voice", value: event.synthetic_probability, interpretation: event.synthetic_probability > 0.6 ? "Characteristics detected" : "Within baseline", tone: event.synthetic_probability > 0.6 ? "critical" : "cyan" }, { field: "speaker_match_score", label: "Speaker match", value: event.speaker_match_score, interpretation: event.speaker_match_score < 0.5 ? "Identity mismatch" : "Enrolled baseline", tone: event.speaker_match_score < 0.5 ? "warning" : "cyan" }, { field: "context_risk_score", label: "Context risk", value: event.context_risk_score, interpretation: event.context_risk_score > 0.6 ? "High-value request" : "Request monitored", tone: event.context_risk_score > 0.6 ? "warning" : "cyan" }] : []; return <aside className="evidence-rail" aria-label="Forensic evidence"><div className="section-heading"><span className="eyebrow">Evidence signals</span><span className="mono-label">{calm ? "BASELINE" : level}</span></div>{calm ? <div className="evidence-calm"><div className="evidence-calm__icon"><Fingerprint size={18} /></div><strong>Signal baseline forming</strong><p>Forensic evidence will appear as the call develops.</p></div> : <div className="evidence-list">{evidence.map((item) => { const unavailable = isUnavailable(item.field); const uncalibrated = item.field === "synthetic_probability" && uncalibratedScore; const displayValue = unavailable ? "N/A" : uncalibrated ? item.value.toFixed(2) : `${Math.round(item.value * 100)}%`; const interpretation = uncalibrated ? "Uncalibrated model score" : item.label === "Speaker match" ? `${100 - Math.round(item.value * 100)}% identity mismatch` : item.interpretation; return <div className="evidence-row" key={item.label}><div className="evidence-row__top"><span>{item.label}</span><strong className={`text-${item.tone}`}>{displayValue}</strong></div><div className="evidence-track"><span className={`evidence-fill evidence-fill--${item.tone}`} style={{ width: unavailable ? "0%" : `${Math.round(item.value * 100)}%` }} /></div><small>{unavailable ? "Not evaluated" : interpretation}</small></div>; })}</div>}</aside>; }
