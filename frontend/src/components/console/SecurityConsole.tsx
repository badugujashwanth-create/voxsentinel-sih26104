...
function CallContextStrip({ context, scenarioLabel, isLive, score, level, timestamp }: { context: DemoController["context"]; scenarioLabel?: string; isLive: boolean; score: number; level: string; timestamp: number }) {
  return <section className="context-strip" aria-label="Call context"><div className="context-cell context-cell--caller"><div className="context-icon"><UserRound size={21} /></div><div><span className="eyebrow">Claimed caller</span><strong>{context?.caller}</strong><small>{context?.role}</small></div></div><div className="context-cell context-cell--request"><div><span className="eyebrow">Sensitive request</span><strong>{context?.request}</strong><small>Urgent vendor transfer</small></div></div><div className="context-cell context-cell--call"><div className="live-label"><span className={`status-dot ${isLive ? "status-dot--green" : "status-dot--muted"}`} /> {isLive ? "LIVE CALL" : "CALL READY"}</div><strong className="mono-metric">{isLive ? formatDuration(timestamp) : "00:00.00"}</strong><small>{scenarioLabel}</small></div><div className={`context-risk context-risk--${level.toLowerCase()}`}><span className="eyebrow">Impersonation risk</span><div><strong>{score}</strong><span>/100</span></div><b>{level}</b></div></section>;
}
...
function IdentityRail({ context, action, isVerified }: { context: DemoController["context"]; action: string; isVerified: boolean }) {
  ...
  return <aside className="identity-rail" aria-label="Identity and action context"><div className="rail-section"><span className="eyebrow">Call identity</span><div className="identity-name"><div className="avatar-mark"><UserRound size={23} /></div><div><strong>{context?.caller}</strong><small>{context?.role}</small></div></div></div><div className="rail-rule" /><div className="rail-section"><span className="eyebrow">Protected action</span><div className="request-amount">â‚¹25,00,000</div><div className="request-label">Vendor transfer</div><div className={`action-chip action-chip--${action.toLowerCase()}`}><span className="status-dot" />{action === "ELIGIBLE" ? "ELIGIBLE TO PROCEED" : action}</div></div><div className="rail-rule" /><div className="rail-section rail-section--small"><span className="eyebrow">Operator note</span><p>{isVerified ? "Identity was confirmed through an independent channel. Voice-risk evidence remains preserved." : "Monitor the voice signal and intervene when the protected action crosses policy threshold."}</p></div></aside>;
}
...
function SignalHeading({ state }: { state: string }) {
  ...
  return <div className="signal-heading"><div><span className="eyebrow eyebrow--cyan"><Activity size={13} /> Live voice forensics</span><h1>{title}</h1></div><span className="signal-spec mono-label">48.0 kHz Â· PCM</span></div>;
}
...
function formatDuration(timestampMs: number): string { const seconds = Math.floor(timestampMs / 1000); const milliseconds = timestampMs % 1000; return `00:${seconds.toString().padStart(2, "0")}.${milliseconds.toString().padStart(3, "0").slice(0, 2)}`; }
...
function EvidenceRail({ event, level, state }: { event: DemoController["currentEvent"]; level: string; state: string }) {
  ...
  const evidence = event ? [
    ...
    { field: "speaker_match_score", label: "Speaker identity", value: event.speaker_match_score, interpretation: event.speaker_state === "INCONSISTENT" ? "Speaker mismatch detected" : event.speaker_state === "CONSISTENT" ? "Expected speaker consistent" : "No enrolled reference", tone: event.speaker_state === "INCONSISTENT" ? "warning" : "cyan", semantics: event.speaker_score_semantics === "uncalibrated_similarity" ? "Uncalibrated speaker similarity" : undefined },
